...
class LanguageSwitcher {
  ...
  async loadTranslations() {
    try {
      ...
      const jsonPath = isPortfolioDetail
        ? "../js/languages.json"
        : "js/languages.json";
  ...
  updateResumeSection(resume) {
    ...
    educationDesc.forEach((p) => {
      ...
      if (
        ...
        text.includes("công nghệ thông tin")
      ) {
    ...
    if (
      companyName &&
      resume.company &&
      companyName.textContent.includes("365")
    ) {
  ...
  updatePortfolioSection(portfolio) {
    const portfolioSpan = document.querySelector(
      "#portfolio .section-title span",
    );
    const portfolioTitle = document.querySelector(
      "#portfolio .section-title h2",
    );
    const portfolioSubtitle = document.querySelector(
